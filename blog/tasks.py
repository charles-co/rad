...
@shared_task
def subscriber_verification(subscriber_id, current_site, email):
    subscriber = Subscriber.objects.get(pk=subscriber_id)
    subject = "Email Verification from rad"
    html = render_to_string("subscription/subscriber.html", {
        'subscriber': subscriber, 
        'domain': current_site, 
        'uid':urlsafe_base64_encode(force_bytes(subscriber.pk)),
        'token': default_token_generator.make_token(subscriber),})
    text = render_to_string("subscription/subscriber.txt", {
        'subscriber': subscriber, 
        'domain': current_site, 
        'uid':urlsafe_base64_encode(force_bytes(subscriber.pk)),
        'token': default_token_generator.make_token(subscriber),})
    send_mail(subject, text, settings.EMAIL_HOST_USER, [email,], html_message=html, fail_silently=False)
    return True
